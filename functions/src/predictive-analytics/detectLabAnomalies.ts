...
export const detectLabAnomalies = functions.https.onCall(async (data: AnomalyDetectionRequest, context) => {
  ...
  try {
    ...
    const statisticalAnomalies = detectStatisticalAnomalies(
      historicalData, 
      baseline, 
      sensitivity
    );
    ...
    const patternAnomalies = detectPatternAnomalies(
      historicalData, 
      sensitivity
    );
...
async function getMetricData(
  ...
  switch (metric) {
    ...
    case 'turnaround_time':
      ...
      const dailyTAT: Record<string, { total: number; count: number }> = {};
...
function calculateBaseline(data: Array<{ date: string; value: number }>) {
...
function detectPatternAnomalies(
  ...
  const dayOfWeekExpected: Record<number, { mean: number; stdDev: number }> = {};
...
function deduplicateAnomalies(anomalies: Anomaly[]): Anomaly[] {
  const uniqueMap = new Map<string, Anomaly>();
