...
export const interpretTestResults = functions.https.onCall(async (data: TestResultData, context) => {
  ...
  try {
    ...
    const followUpTests = await generateFollowUpRecommendations(
      testResult, 
      testDefinition, 
      structuredResult.relatedConditions
    );
...
function parseAIResponse(aiResponse: string, testResult: any, testDefinition: any): any {
  try {
...
function extractCriticalFindings(text: string, testResult: any, testDefinition: any): string[] {
...
function extractRelatedConditions(text: string): any[] {
