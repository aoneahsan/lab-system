...
interface QCPredictionData {
  testCode: string;
  historicalQCData: Array<{
    date: string;
    level1: number;
    level2: number;
    level3?: number;
...
export const predictQCResults = functions.https.onCall(async (request: functions.https.CallableRequest<QCPredictionData>) => {
  ...
  try {
    ...
    const sortedData = [...historicalQCData].sort((a, b) => 
    ...
    const maintenanceAlert = checkMaintenanceNeeds(
      qcAnalysis, 
      equipmentInfo,
      qcEvaluation.predictedResult
    );
    
    // Generate AI recommendations
    const recommendations = await generateQCRecommendations(
      testCode,
      qcAnalysis,
      qcEvaluation,
      equipmentInfo
...
function analyzeQCHistory(qcData: any[]) {
...
function predictNextQCValues(qcData: any[]) {
...
function evaluateQCPrediction(predictedValues: any, qcAnalysis: any) {
  let violations = 0;
  ...
  Object.entries(predictedValues).forEach(([level, value]) => {
    ...
    const zScore = Math.abs((value as number - targetMean) / targetSD);
  ...
  let predictedResult: 'pass' | 'fail' | 'warning';
  if (violations > 0) {
...
function checkMaintenanceNeeds(
  ...
  Object.entries(qcAnalysis.trends).forEach(([level, trend]: [string, any]) => {
  ...
  Object.entries(qcAnalysis.volatility).forEach(([level, cv]) => {
    if ((cv as number) > 15) {
      issues.push(`High variability in ${level} (CV: ${(cv as number).toFixed(1)}%)`);
    }
  });
...
async function generateQCRecommendations(
  testCode: string,
  qcAnalysis: any,
  qcEvaluation: any,
  equipmentInfo?: any
