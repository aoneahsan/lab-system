...
exports.assessPatientRisk = void 0;
...
exports.assessPatientRisk = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const aiAssessment = await generateAIRiskAssessment(patientInfo, recentResults, allRiskFactors, medicalHistory);
...
function analyzeDemographics(patientInfo) {
    const riskFactors = [];
...
function checkHighRiskCombinations(resultsByTest, riskFactors) {
...
function generateHealthAlerts(riskFactors, recentResults) {
    const alerts = [];
    // Critical value alerts
    const criticalResults = recentResults.filter(r => r.flag === 'critical_high' || r.flag === 'critical_low');
...
async function generateAIRiskAssessment(patientInfo, recentResults, riskFactors, medicalHistory) {
    const age = calculateAge(new Date(patientInfo.dateOfBirth));
...
function generateMonitoringPlan(riskFactors, riskScore) {
...
//# sourceMappingURL=assessPatientRisk.js.map