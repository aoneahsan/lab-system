...
exports.predictResourceNeeds = void 0;
...
exports.predictResourceNeeds = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const forecasts = await Promise.all(Object.entries(dataByResource).map(async ([resourceKey, data]) => {
            ...
            const predictions = generateUtilizationForecast(utilizationData, forecastDays, trend, seasonality);
            ...
            const stockoutRisk = assessStockoutRisk(predictedUtilization, trend, resourceType);
            ...
            const recommendations = await generateResourceRecommendations(resourceType, resourceId, recentUtilization, predictedUtilization, stockoutRisk, trend);
            return {
                resourceType,
                resourceId,
...
function detectUtilizationTrend(data) {
    const n = data.length;
...
function detectSeasonality(data) {
    ...
    if (weeklyPattern.strength > 0.5) {
        return { pattern: 'weekly', strength: weeklyPattern.strength };
    }
    // Check for monthly pattern
    if (data.length >= 60) {
        const monthlyPattern = checkCyclicPattern(values, 30);
        if (monthlyPattern.strength > 0.5) {
            return { pattern: 'monthly', strength: monthlyPattern.strength };
        }
    }
    return { pattern: null, strength: 0 };
}
function checkCyclicPattern(values, period) {
...
function generateUtilizationForecast(historicalData, forecastDays, trend, seasonality) {
    const predictions = [];
...
function calculateDayOfWeekAverage(data, dayOfWeek) {
...
function assessStockoutRisk(predictedUtilization, trend, resourceType) {
...
function calculateReorderDate(historicalData, predictions, stockoutRisk) {
    if (stockoutRisk === 'low')
        return undefined;
...
async function generateResourceRecommendations(resourceType, resourceId, currentUtilization, predictedUtilization, stockoutRisk, trend) {
...
//# sourceMappingURL=predictResourceNeeds.js.map