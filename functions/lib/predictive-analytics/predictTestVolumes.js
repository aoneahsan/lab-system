...
exports.predictTestVolumes = void 0;
...
exports.predictTestVolumes = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const forecasts = await Promise.all(Object.entries(dataByTest).map(async ([testCode, data]) => {
            ...
            const recommendations = await generateRecommendations(testCode, sortedData, predictions, stats, trend);
            return {
                testCode,
...
function generateForecast(historicalData, forecastDays, trend, seasonality) {
    const predictions = [];
...
function calculateWeeklyAverage(data, dayOfWeek) {
    const dayData = data.filter(d => new Date(d.date).getDay() === dayOfWeek);
    if (dayData.length === 0)
        return 0;
    return dayData.reduce((sum, d) => sum + d.count, 0) / dayData.length;
}
async function generateRecommendations(testCode, historicalData, predictions, stats, trend) {
...
//# sourceMappingURL=predictTestVolumes.js.map