...
export default function QCResultEntry() {
  ...
  const [lastResult, setLastResult] = useState<{ violations: string[], status: string } | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const status = result.violations.length === 0 ? 'accepted' :
        result.violations.includes('1_3s') || result.violations.includes('2_2s') || 
        result.violations.includes('R_4s') || result.violations.includes('4_1s') || 
        result.violations.includes('10x') ? 'rejected' : 'warning';
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {showResult && lastResult && (
        <div className={`bg-white p-6 rounded-lg shadow-sm border ${
          lastResult.status === 'accepted' ? 'border-green-200' :
          lastResult.status === 'warning' ? 'border-yellow-200' : 'border-red-200'
        }`}>
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h4 className={`font-medium text-lg mb-2 ${
                lastResult.status === 'accepted' ? 'text-green-900' :
                lastResult.status === 'warning' ? 'text-yellow-900' : 'text-red-900'
              }`}>
                QC Result {lastResult.status === 'accepted' ? 'Accepted' :
                         lastResult.status === 'warning' ? 'Warning' : 'Rejected'}
              </h4>
