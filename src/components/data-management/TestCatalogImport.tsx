...
interface ImportProgress {
  ...
  successful: number;
  failed: number;
  status: 'idle' | 'validating' | 'mapping' | 'importing' | 'complete' | 'error';
}
...
export const TestCatalogImport: React.FC = () => {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      if (validation.isValid) {
        setProgress({ ...progress, status: 'mapping' });
        
        // Map LOINC codes
        const mappedData = await Promise.all(
          validation.validRows.map(async (row) => {
            const loincMappings = await LOINCMapper.mapTestToLOINC(row);
            return {
              ...row,
              suggestedLOINC: loincMappings[0] || null,
              loincMappings,
            };
          })
        );
        
        setMappingPreview(mappedData);
        setProgress({ ...progress, status: 'idle', total: mappedData.length });
      } else {
  ...
  const handleImport = async () => {
    if (!mappingPreview.length || !currentTenant) return;
    ...
    toast.success(`Import complete: ${progress.successful} tests imported successfully`);
  };
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-gray-600">
          {isDragActive
            ? 'Drop the file here...'
            : 'Drag & drop your test catalog file here, or click to select'}
        </p>
      ...
      {/* Import Button */}
      {mappingPreview.length > 0 && progress.status === 'idle' && (
        <div className="flex justify-end">
          <Button onClick={handleImport} disabled={!validationResult?.isValid}>
            Import {mappingPreview.length} Tests
          </Button>
        </div>
      )}
    </div>
