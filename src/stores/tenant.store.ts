	...
	persist(
		(set, get) => ({
			...
			setCurrentTenant: (tenant) =>
				set({ currentTenant: tenant, lastFetch: Date.now() }),
			setLoading: (isLoading) => set({ isLoading }),
			setError: (error) => set({ error }),
			clearTenant: () =>
				set({ currentTenant: null, error: null, lastFetch: null }),
			...
			fetchTenant: async (tenantId) => {
				...
				try {
					...
					const tenantDoc = await getDoc(
						doc(firestore, TENANT_COLLECTION, tenantId)
					);
					...
					const tenantData = {
						...
						subscription: {
							...
							startDate:
								tenantDoc.data().subscription?.startDate?.toDate() ||
								new Date(),
