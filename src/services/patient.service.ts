...
class PatientService {
	...
	private formatPatientData(data: Record<string, unknown>): Patient {
		...
		return {
			...
			insurances:
				(data.insurances as Record<string, unknown>[] | undefined)?.map(
					(ins) => ({
						...ins,
			...
			allergies:
				(data.allergies as Record<string, unknown>[] | undefined)?.map(
					(allergy) => ({
						...allergy,
			...
			medications:
				(data.medications as Record<string, unknown>[] | undefined)?.map(
					(med) => ({
						...med,
			...
			medicalHistory:
				(data.medicalHistory as Record<string, unknown>[] | undefined)?.map(
					(history) => ({
						...history,
			...
			documents:
				(data.documents as Record<string, unknown>[] | undefined)?.map(
					(doc) => ({
						...doc,
		...
		try {
			...
			snapshot.docs.forEach((doc) => {
				...
				if (data.gender) {
					stats.genderDistribution[
						data.gender as keyof typeof stats.genderDistribution
					]++;
				}
			...
			stats.averageAge =
				stats.totalPatients > 0
					? Math.round(totalAge / stats.totalPatients)
					: 0;
