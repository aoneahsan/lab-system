...
class PatientService {
  ...
  private formatPatientData(data: Record<string, unknown>): Patient {
    ...
    return {
      ...
      insurances: (data.insurances as Record<string, unknown>[] | undefined)?.map((ins) => ({
        ...ins,
        validFrom: toDate(ins.validFrom) || new Date(),
        validTo: toDate(ins.validTo),
      })) || [],
      allergies: (data.allergies as Record<string, unknown>[] | undefined)?.map((allergy) => ({
        ...allergy,
        confirmedDate: toDate(allergy.confirmedDate),
      })) || [],
      medications: (data.medications as Record<string, unknown>[] | undefined)?.map((med) => ({
        ...med,
        startDate: toDate(med.startDate) || new Date(),
        endDate: toDate(med.endDate),
      })) || [],
      medicalHistory: (data.medicalHistory as Record<string, unknown>[] | undefined)?.map((history) => ({
        ...history,
        diagnosedDate: toDate(history.diagnosedDate),
      })) || [],
      documents: (data.documents as Record<string, unknown>[] | undefined)?.map((doc) => ({
        ...doc,
  ...
  private formatPatientListItem(patient: Patient): PatientListItem {
    return {
      ...
      phoneNumber: patient.phoneNumbers.find(p => p.isPrimary)?.value || patient.phoneNumbers[0]?.value || '',
      email: patient.email,
  ...
  async createPatient(tenantId: string, data: CreatePatientData, createdBy: string): Promise<Patient> {
    try {
  ...
  async updatePatient(tenantId: string, patientId: string, data: UpdatePatientData): Promise<Patient> {
    try {
      const collectionName = this.getCollectionName(tenantId);
