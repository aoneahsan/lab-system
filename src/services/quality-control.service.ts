...
const WESTGARD_RULES: QCRule[] = [
  {
    ...
    name: '1-2s',
    code: '1_2s',
    description: 'One control observation exceeds ±2SD',
    type: 'warning',
    enabled: true,
  },
  {
    ...
    name: '1-3s',
    code: '1_3s',
    description: 'One control observation exceeds ±3SD',
    type: 'rejection',
    enabled: true,
  },
  {
    ...
    name: '2-2s',
    code: '2_2s',
    description: 'Two consecutive values exceed same ±2SD limit',
    type: 'rejection',
    enabled: true,
  },
  {
    ...
    name: 'R-4s',
    code: 'R_4s',
    description: 'One observation exceeds +2SD and another exceeds -2SD',
    type: 'rejection',
    enabled: true,
  },
  {
    ...
    name: '4-1s',
    code: '4_1s',
    description: 'Four consecutive values exceed same ±1SD limit',
    type: 'rejection',
    enabled: true,
  },
  {
    ...
    name: '10x',
    code: '10x',
    description: 'Ten consecutive values are on the same side of the mean',
    type: 'rejection',
    enabled: true,
  },
];
...
export const qualityControlService = {
  ...
  async recordQCResult(
    ...
    const acceptanceStatus =
      violations.length === 0
        ? 'accepted'
        : violations.some((v) => WESTGARD_RULES.find((r) => r.code === v)?.type === 'rejection')
          ? 'rejected'
          : 'warning';
  ...
  async getQCResults(qcTestId: string, levelId?: string, days: number = 30): Promise<QCResult[]> {
    ...
    const q = query(collection(db, COLLECTIONS.QC_RESULTS), ...constraints);
  ...
  async getLeveyJenningsData(qcTestId: string, levelId: string, days: number = 30): Promise<any[]> {
    const results = await this.getQCResults(qcTestId, levelId, days);
    const qcTest = await this.getQCTest(qcTestId);
    const level = qcTest?.levels.find((l) => l.id === levelId);
