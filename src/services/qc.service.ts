...
export const qcService = {
  ...
  async getQCMaterials(tenantId: string): Promise<QCMaterial[]> {
    ...
    return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as QCMaterial));
  },

  async getQCMaterial(tenantId: string, materialId: string): Promise<QCMaterial | null> {
  ...
  async createQCMaterial(
    ...
    const materialData: Omit<QCMaterial, 'id'> = {
      ...
      openedDate: data.openedDate ? Timestamp.fromDate(data.openedDate) : undefined,
      active: true,
    ...
    const docRef = await addDoc(collection(db, COLLECTIONS.QC_MATERIALS), materialData);
    return docRef.id;
  },
  ...
  async getQCRuns(tenantId: string, filter?: QCFilter): Promise<QCRun[]> {
    ...
    if (filter?.testCode) {
      q = query(q, where('results', 'array-contains-any', [{ testCode: filter.testCode }]));
    }
    if (filter?.level) {
  ...
  async getQCStatistics(
    ...
    return { id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as QCStatistics;
  },
  ...
  async updateQCStatistics(
    ...
    for (const analyte of material.analytes) {
      ...
      const bias = ((stats.mean - analyte.targetMean) / analyte.targetMean) * 100;
      const totalError = Math.abs(bias) + 1.65 * stats.cv;
      const sigma = analyte.targetCV ? (analyte.targetCV - Math.abs(bias)) / stats.cv : 0;
  ...
  async getQCDashboardStats(tenantId: string): Promise<QCDashboardStats> {
    ...
    const allRuns = allRunsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as QCRun));
    
    // Calculate statistics
    const totalRuns = allRuns.length;
    const passedRuns = allRuns.filter(run => run.status === 'accepted' || run.status === 'completed').length;
    const failedRuns = allRuns.filter(run => run.status === 'rejected').length;
    const pendingReview = allRuns.filter(run => run.status === 'completed' && !run.reviewedBy).length;
    ...
    levels.forEach(level => {
      ...
      const levelPassed = levelRuns.filter(run => run.status === 'accepted' || run.status === 'completed').length;
      byLevel[level] = {
        runs: levelRuns.length,
        passRate: levelRuns.length > 0 ? (levelPassed / levelRuns.length) * 100 : 0,
      };
    });
    
    // By test statistics
    const testMap = new Map<string, { name: string; runs: number; passed: number; values: number[] }>();
    ...
    const recentViolations = allRuns
      ...
      .map(run => ({
        runId: run.id,
        runDate: run.runDate,
        testName: run.results.find(r => r.violatedRules.length > 0)?.testName || '',
        level: run.level,
    ...
    const materials = materialsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as QCMaterial));
    
    const expiringMaterials = materials
      .map(material => {
        const daysUntilExpiration = Math.floor(
          (material.expirationDate.toDate().getTime() - Date.now()) / (1000 * 60 * 60 * 24)
        );
        return {
          id: material.id,
          name: material.name,
