...
export const useBiometricAuth = () => {
	...
	const [biometricStatus, setBiometricStatus] =
		useState<BiometricEnrollmentStatus | null>(null);
	const [preferences, setPreferences] = useState<BiometricPreferences | null>(
		null
	);
	...
	const authenticate = useCallback(
		async (
			config?: Partial<BiometricAuthConfig>
	...
	const updatePreferences = useCallback(
		async (newPreferences: Partial<BiometricPreferences>): Promise<void> => {
			try {
				setIsLoading(true);
				const currentPrefs =
					preferences || (await biometricService.getPreferences());
