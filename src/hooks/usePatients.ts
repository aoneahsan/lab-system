	...
	return useMutation({
		...
		onSuccess: (patient) => {
			...
			queryClient.setQueryData(
				['patient', currentTenant?.id, patient.id],
				patient
			);
...
export const useDeactivatePatient = () => {
	...
	return useMutation({
		mutationFn: (patientId: string) => {
			...
			return patientService.deactivatePatient(
				currentTenant.id,
				patientId,
				currentUser.id
			);
...
export const useActivatePatient = () => {
	...
	return useMutation({
		mutationFn: (patientId: string) => {
			...
			return patientService.activatePatient(
				currentTenant.id,
				patientId,
				currentUser.id
			);
		...
		onError: (error) => {
			showToast({
				...
				message:
					error instanceof Error ? error.message : 'Failed to activate patient',
			});
		},
	});
};
