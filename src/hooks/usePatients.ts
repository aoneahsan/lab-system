...
export const usePatients = (filters: PatientSearchFilters = {}, pageSize = 20) => {
  ...
  return useQuery({
    ...
    queryFn: () => {
      ...
      return performanceMonitor.trackApiCall('patients_search', () =>
...
export const usePatient = (patientId: string) => {
  ...
  return useQuery({
    ...
    queryFn: () => {
      if (!currentTenant?.id) throw new Error('No tenant selected');
      return performanceMonitor.trackApiCall('patient_get', () =>
...
export const useCreatePatient = () => {
  ...
  return useMutation({
    mutationFn: (data: CreatePatientData) => {
      ...
      return performanceMonitor.trackApiCall('patient_create', () =>
        patientService.createPatient(currentTenant.id, data, currentUser.id)
      );
    },
    onSuccess: (patient) => {
